...
def adjust_learning_rate(optimizer, epoch, warmup_epoches, milestones, init_lr):
    if epoch <= warmup_epoches:
        lr = init_lr * (epoch / warmup_epoches)
...
if __name__ == '__main__':
    ...
    if not opt.no_train:
        for i in range(opt.begin_epoch, opt.n_epochs + 1):
            ...
            adjust_learning_rate(optimizer, i, 3, opt.milestones, opt.learning_rate)
