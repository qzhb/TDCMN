...
class Crossdomain_Dynamic_Attention(nn.Module):
    ...
    def forward(self, features):
        ...
        return [domain_one_atten_feat, domain_two_atten_feat, domain_time_atten_feat]
...
class Event_Model(nn.Module):
    ...
    def __init__(self, opt):
        ...
        # cross domain
        self.crossdomain_dynamic_attention = Crossdomain_Dynamic_Attention(classes=[self.concept_number, self.scene_classes, self.object_classes], branches=self.branches, domains = self.domains, T=self.T)
    ...
    def forward(self, sceobj_frame):
        ...
        crossdomain_scene_feature = torch.matmul(crossdomain_scene_feature, attentions[2][:, 0, :, :])

        ## object dynamic feature 
        crossdomain_object_feature = torch.sum(object_feature_b*attentions[1], dim=1)
        crossdomain_object_feature = torch.matmul(crossdomain_object_feature, attentions[2][:, 1, :, :])

        ## concat
        indomain_feature = torch.cat((indomain_scene_feature, indomain_object_feature), 1).squeeze(2)
        crossdomain_feature = torch.cat((crossdomain_scene_feature, crossdomain_object_feature), 1).squeeze(2)
